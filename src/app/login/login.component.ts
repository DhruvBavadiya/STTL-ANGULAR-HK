  ...
  handleSubmit(): void {
    ...
    this.authService.loginUser(this.email, this.password).subscribe(
      (userData) => {
        ...
        this.cartService.getCart(userData._id).subscribe(
          (data) => {
            const dbdata = data.cartitems; // Assign fetched data to dbdata
            const cartItems: cartProduct[] =
              typeof window !== 'undefined'
                ? JSON.parse(localStorage.getItem('cartItems') || '[]')
                : [];
  
            // Call BindData with fetched data and local storage data
            const finalData = this.mergeCartItems(dbdata, cartItems,userData._id);
            console.log(dbdata , cartItems ,finalData)
            localStorage.setItem('cartItems', JSON.stringify(finalData));
              this.cartService.updateCart(userData._id, finalData).subscribe(
                () => {
